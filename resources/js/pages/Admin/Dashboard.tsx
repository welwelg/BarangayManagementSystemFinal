...
export default function Dashboard({ stats = {} }) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl bg-sky-50 p-4 dark:bg-gray-950">
                ...
                <div className="grid gap-4 md:grid-cols-4">
                    <Card className="border-1 border-sky-300 bg-gradient-to-r from-sky-100 to-sky-300 shadow-md transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg dark:border-sky-800 dark:from-sky-700 dark:to-sky-900">
                        <CardContent className="p-4">
                            ...
                            <div className="mt-2">
                                <span className="text-2xl font-bold text-sky-950 dark:text-white">{male_residents}</span>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-1 border-pink-300 bg-gradient-to-r from-pink-100 to-pink-300 shadow-md transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg dark:border-pink-800 dark:from-pink-700 dark:to-pink-900">
                        <CardContent className="p-4">
                            ...
                            <div className="mt-2">
                                <span className="text-2xl font-bold text-pink-500 dark:text-white">{female_residents}</span>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-1 border-green-300 bg-gradient-to-r from-green-100 to-green-300 shadow-md transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg dark:border-green-800 dark:from-green-700 dark:to-green-900">
                        <CardContent className="p-4">
                            ...
                            <div className="mt-2">
                                <span className="text-2xl font-bold text-green-500 dark:text-white">{average_age}</span>
                                <span className="ml-1 text-sm text-gray-400 dark:text-white">years</span>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-1 border-purple-300 bg-gradient-to-r from-purple-100 to-purple-300 shadow-md transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg dark:border-purple-800 dark:from-purple-700 dark:to-purple-900">
                        <CardContent className="p-4">
                            ...
                            <div className="mt-2">
                                <span className="text-2xl font-bold text-purple-500 dark:text-white">{stats.total_zones ?? 0}</span>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                            ...
                            {Object.keys(residents_by_zone).length > 0 ? (
                                <div className="space-y-3">
                                    {Object.entries(residents_by_zone).map(([zone, count]) => (
                                        ...
                                        >
                                            <div>
                                                <p className="font-semibold text-sky-950 dark:text-white">{zone}</p>
                                                <p className="text-sm text-sky-900 dark:text-white">Zone residents</p>
                                            </div>
                                            <div className="text-right">
                                                <span className="text-xl font-bold text-sky-950 dark:text-white">{count}</span>
                            ...
                            {stats.latest_announcements && stats.latest_announcements.length > 0 ? (
                                <div className="space-y-3">
                                    {stats.latest_announcements.slice(0, 5).map((announcement) => (
                                        ...
                                        >
                                            <h3 className="font-semibold text-sky-950 dark:text-white">{announcement.title}</h3>
                                            <p className="truncate text-sm text-sky-900 dark:text-white">{announcement.message}</p>
                                ...
                                <div className="space-y-3">
                                    {recent_activities.map((activity, index) => (
                                        <div key={index} className="rounded-lg bg-sky-200 p-3 shadow-sm transition hover:shadow-md dark:bg-sky-950">
                                            <p className="font-semibold text-sky-950 dark:text-white">{activity.title}</p>
                                            <p className="truncate text-sm text-sky-900 dark:text-white">{activity.description}</p>
