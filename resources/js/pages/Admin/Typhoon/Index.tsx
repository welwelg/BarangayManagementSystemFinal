...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Typhoon Monitor', href: '/admin/typhoon-monitoring' }];
...
export default function Index() {
    ...
    // In the fetchData useCallback:
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const typhoonRes = await fetch('/admin/typhoon/data');
            const typhoonData = await typhoonRes.json();
            setTyphoons(typhoonData);
            if (userLocation) {
                const forecastRes = await fetch(`/admin/typhoon/forecast?lat=${userLocation.lat}&lon=${userLocation.lon}`);
                const forecastData = await forecastRes.json();
                setForecast(forecastData);
            } else {
                const forecastRes = await fetch('/admin/typhoon/forecast');
                const forecastData = await forecastRes.json();
                setForecast(forecastData);
            }
        } finally {
